...
int								env_set(t_var **env, const char *name,
									char *value);
...
bool							add_token(char *value, t_token_type type,
									t_tok_list *list);
...
t_node							*parsing(t_token *head, t_var *env, int status,
									int *err);
t_node							*parse_pipeline(t_token **cur,
									t_parse_info *info);
t_node							*parse_command(t_token **cur,
									t_parse_info *info);
t_node							*new_cmd_node(void);
t_node							*syntax_err_node(t_node *node,
									t_parse_info *info);
t_redirect						*create_redirect(t_token *op, t_token *target,
									t_parse_info *info);
int								add_arg(t_node *node, char *value);
int								add_word(t_node *node, t_token *tok,
									t_parse_info *info);
int								process_redir(t_node *cmd_node, t_token **cur,
									t_parse_info *info);
void							append_redirect(t_node *cmd_node,
									t_redirect *r);
...
t_node							*parse_primary(t_token **cur,
									t_parse_info *info);
t_node							*new_op_node(t_node_type type, t_node *left,
									t_node *right);
...
int								run_tree(t_node *root, t_var **env,
									int last_status);
...
void							exec_node_in_child(t_node *root, t_node *cur,
									t_var **env);
void							exec_external_cmd(t_node *root, t_node *cur,
									t_var **env);
...
void							cleanup_and_exit(t_node *root, t_var **env,
									int code);
...
int								err_msg(const char *prefix, const char *msg,
									int code);
...
// heredoc
int								process_all_heredocs(t_node *node, t_var *env,
									int status);
